                  ...
                  <div className="flex-shrink-0">
                    <div className="w-24 h-24 rounded-full bg-gray-700 overflow-hidden border-2 border-gray-600">
                      {user?.profile_picture || previewUrl ? (
                        <img
                          src={previewUrl || `${getApiBaseUrl().replace('/api', '')}${user?.profile_picture?.startsWith('/') ? '' : '/'}${user?.profile_picture || ''}`}
                          ...
                          onError={(e) => {
                            ...
                            if (parent) {
                              parent.innerHTML = '<div class="w-full h-full flex items-center justify-center"><svg class="w-12 h-12 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg></div>';
                            }
                          }}
                        />
                      ) : (
