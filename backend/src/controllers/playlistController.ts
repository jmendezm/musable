...
export const getAllPlaylists = asyncHandler(async (req: AuthRequest, res: Response) => {
...
export const getPlaylist = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const playlistId = parseInt(id);
  const userId = req.user?.id;

  const canAccess = await PlaylistModel.canUserAccessPlaylist(playlistId, userId);
  if (!canAccess) {
...
export const reorderPlaylistSongs = asyncHandler(async (req: AuthRequest, res: Response) => {
...
export const followPlaylist = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const playlistId = parseInt(id);
  const userId = req.user!.id;
...
export const unfollowPlaylist = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const playlistId = parseInt(id);
  const userId = req.user!.id;
...
export const togglePlaylistFollow = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const playlistId = parseInt(id);
  const userId = req.user!.id;
  ...
  const result = await PlaylistFollowsModel.togglePlaylistFollow(userId, playlistId);
...
export const getFollowedPlaylists = asyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.user!.id;
  const playlists = await PlaylistFollowsModel.getUserFollowedPlaylists(userId);
...
export const getPlaylistsWithFollowStatus = asyncHandler(async (req: AuthRequest, res: Response) => {
  const userId = req.user!.id;
  const includeOwn = req.query.includeOwn !== 'false';
  const limit = parseInt(req.query.limit as string) || 50;
  const offset = parseInt(req.query.offset as string) || 0;

  const playlists = await PlaylistFollowsModel.getPlaylistsWithFollowStatus(userId, includeOwn, limit, offset);
...
export const getPlaylistFollowStatus = asyncHandler(async (req: AuthRequest, res: Response) => {
  const { id } = req.params;
  const playlistId = parseInt(id);
  const userId = req.user!.id;

  const isFollowing = await PlaylistFollowsModel.isFollowingPlaylist(userId, playlistId);
