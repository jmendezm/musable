...
export const getSharedSong = async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    const publicSharingEnabled = await SettingsModel.getSetting('public_sharing_enabled');
    ...
    logger.info(`Song accessed via share token: ${result.song.title} by ${result.song.artist_name} (Token: ${token.substring(0, 8)}...)`);
...
export const createShareToken = asyncHandler(async (req: AuthRequest, res: Response) => {
